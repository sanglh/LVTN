	...
	REAL :: bondSiC = 1.78 ! Bond lenght Si-C
	...
	periodY = bondSiC * (MCOS(60.0) + 1 + MCOS(60.0) + 1)
	periodX = bondSiC * 2 * MSIN(60.0)
	! COLUMN 1 : Si
	initX = 0
	initY = 0
	...
	! COLUMN 3 : Si
	initX = bondSiC * (MSIN(60.0))
	initY = bondSiC * (MCOS(60.0) + 1)
	CALL createCoordinate(initX, periodX, initY, periodY, x1, y1, z1, nodeX, nodeY, cntSi)
	! COLUMN 4 : C
	initX = bondSiC * (MSIN(60.0))
	initY = bondSiC * (MCOS(60.0) + 1 + 1)
	CALL createCoordinate(initX, periodX, initY, periodY, x2, y2, z2, nodeX, nodeY, cntC)
	! Write lammps file
